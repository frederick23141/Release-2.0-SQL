...
Public Class Ppto_produccionLn
    ...
    Public Sub add_Cantidad_Inv(ByRef dt As DataTable, ByVal centro As Integer, ByVal where_codigo As String, ByVal mes As Integer, ByVal ano As Integer)
        ...
        Next
    End Sub
    Public Function cargar_seguimiento_gral_areaGrafica(ByVal dias_habiles_otros As Double, ByVal dias_adic_otros As Double, ByVal dias_trabajados_otros As Double, ByVal mes As Integer, ByVal ano As Integer, ByVal centro As Integer) As DataTable
        Dim dias_habiles As Double = 0
        Dim dias_adic As Double = 0
        Dim cant_festivos As Integer = 0
        Dim dias_trbajados As Double = 0
        Dim dt As New DataTable
        'Dim sql As String = "SELECT c.centro,c.descripcion,p.kilos As ppto_kilos,p.cantidad as ppto_cantidad,dias_habiles,dias_adicionales,mes " &
        '                                    "FROM J_ppto_prod_area p , CORSAN.dbo.centros  c " &
        '                                        "WHERE p.centro = c.centro AND ano = " & ano & " " &
        '                                           "ORDER BY p.centro "
        Dim sql As String = "SELECT centro,kilos As ppto_kilos,cantidad as ppto_cantidad,dias_habiles,dias_adicionales,mes " &
                                            "FROM J_ppto_prod_area" &
                                                "WHERE centro = 2100 AND ano = 2021 " &
                                                   "ORDER BY centro "
    ...
    Public Function cargar_areaGrafica(ByVal ano As Integer, ByVal centro As Integer) As DataTable
        ...
        'Dim sql As String = "SELECT SUM(kilos) AS kilos, ano, mes " &
        '                                    "FROM dbo.J_transacciones_kilos " &
        '                                        "WHERE ano = " & ano & " " & where_sql_inv & " " &
        '                                           "GROUP BY ano,mes "
    ...
    Public Function cargar_areaGraficacostos(ByVal ano As Integer, ByVal centro As Integer) As DataTable


        Dim where_centro As String = " AND centro IN ( 2100) "
        ...
        Dim sql As String = "SELECT p.mes,p.ano,t.descripcion As tipo,c.cuenta ,c.descripcion As desc_cuenta ,(SELECT SUM (ppto) FROM J_ppto_gastos WHERE cuenta = c.cuenta  AND p.mes = mes AND ano =  p.ano " & where_centro & " ) As presupuesto ,  (SELECT SUM (valor) FROM movimiento WHERE cuenta = c.cuenta AND Year(fec) = p.ano AND MONTH(fec) = p.mes " & where_centro & " ) As saldo " &
                             "FROM j_ppto_gastos_tipo t, J_ppto_gastos_cuentas g , cuentas c , J_ppto_gastos p    " &
                                "WHERE g.tipo = t.id And g.cuenta = c.cuenta And p.cuenta = g.cuenta  And p.ano = " & ano & " " &
    ...
    Public Function cargar_seguimiento_gral_area(ByVal dias_habiles_otros As Double, ByVal dias_adic_otros As Double, ByVal dias_trabajados_otros As Double, ByVal mes As Integer, ByVal ano As Integer, ByVal centro As Integer) As DataTable
        Dim dias_habiles As Double = 0
        Dim dias_adic As Double = 0
        Dim cant_festivos As Integer = 0
        Dim dias_trbajados As Double = 0
        ...
        Dim sql As String = "SELECT c.centro,c.descripcion,p.kilos As ppto_kilos,p.cantidad as ppto_cantidad,dias_habiles,dias_adicionales " &
                                            "FROM J_ppto_prod_area p , CORSAN.dbo.centros  c " &
                                                "WHERE p.centro = " & centro & " AND mes =" & mes & " AND ano = " & ano & " " &
                                                   "ORDER BY p.centro "
        ...
        dt.Columns.Add("pxn_a_recup", GetType(Double))
        dt.Columns.Add("cump", GetType(Double))
        'dt.Columns.Add("iC", GetType(Image))
        ...
        For i = 0 To dt.Rows.Count - 1
            ...
            add_inv_area(dt, dt.Rows(i).Item("centro"), where_sql_inv, mes, ano)
            add_Cantidad_Inv(dt, dt.Rows(i).Item("centro"), where_sql_inv, mes, ano)
            ' dt.Rows(i).Item("iC").Value = Spic.My.Resources.check_ok
        Next
        calcular_cumplimiento_area(dt, dias_habiles, dias_trbajados, dias_habiles_otros, dias_adic_otros, dias_trabajados_otros)
        Return dt
    End Function
    ...
    Public Function cargar_seguimiento_gral_area1(ByVal dias_habiles_otros As Double, ByVal dias_adic_otros As Double, ByVal dias_trabajados_otros As Double, ByVal mes As Integer, ByVal ano As Integer, ByVal centro As Integer) As DataTable
        Dim dias_habiles As Double = 0
        Dim dias_adic As Double = 0
        Dim cant_festivos As Integer = 0
        Dim dias_trbajados As Double = 0
        ...
        'Dim sql As String = "SELECT centro,kilos As ppto_kilos,cantidad as ppto_cantidad,dias_habiles,dias_adicionales,mes " &
        '                                    "FROM J_ppto_prod_area " &
        '                                        "WHERE  centro = " & centro & "  AND ano = " & ano & " " &
        '                                           "ORDER BY centro "
        Dim sql As String = "SELECT centro,kilos As ppto_kilos,cantidad as ppto_cantidad,dias_habiles,dias_adicionales,mes " &
                                            "FROM J_ppto_prod_area " &
                                                "WHERE  centro = " & centro & "  AND ano = 2021 " &
                                                   "ORDER BY centro "
    ...
    Private Sub add_extra(ByRef dt As DataTable, ByVal mes As Integer, ByVal ano As Integer)
        ...
        Dim sql_extra As String = "SELECT y.centro,SUM(y.horas)As horas,AVG(p.presupuesto)As presupuesto " &
                        " FROM y_liquidacion y , y_conceptos_nom c ,PRGPRODUCCION.dbo.J_ppto_prod_extra p " &
                            "WHERE p.mes = y.mes AND p.ano = y.ano AND y.centro = p.centro AND (y.mes) = " & mes & " And (y.ano)  = " & ano & " AND  y.concepto = c.concepto AND y.concepto IN ('13', '14', '15', '16')  " &
                                "GROUP BY y.centro " &
                                    "ORDER BY y.centro "
    ...
    Private Sub add_eficiencias(ByRef dt As DataTable, ByVal mes As Integer, ByVal ano As Integer)
        ...
        For i = 0 To dt.Rows.Count - 1
            If IsNumeric(dt.Rows(i).Item("centro")) Then
                ...
                Select Case centro
                    Case 2100
                        sql = "SELECT YEAR(fecha)As ano,MONTH(fecha)As mes,((SUM (prod.reproceso) + SUM (prod.alambron)) / SUM(prod.kilos_esp)) * 100 AS efic " &
                                    "FROM j_v_tref_completo prod  " &
                                        "WHERE YEAR(fecha)= " & ano & " AND MONTH(fecha)= " & mes & " " &
                                            "GROUP BY YEAR(fecha),MONTH(fecha)"
                    Case 6400
                        sql = "	SELECT YEAR(fecha)As ano,MONTH(fecha)As mes,(SELECT CASE WHEN SUM (prod.kilos_esp)= 0 THEN 0 ELSE(((SUM (prod.kilos_prod ))/SUM (prod.kilos_esp))*100)END)As efic  " &
                                    "FROM J_prod_puas prod  " &
                                        "WHERE YEAR(fecha)= " & ano & " AND MONTH(fecha)= " & mes & " " &
                                            "GROUP BY YEAR(fecha),MONTH(fecha)"
                    Case 2300
                        sql = "	SELECT YEAR(fecha)As ano,MONTH(fecha)As mes,(((SUM (kilos_prod ))/SUM (kil_esperado))*100)As efic  " &
                                    "FROM J_prod_puntilleria prod " &
                                                  "WHERE YEAR(fecha)= " & ano & " AND MONTH(fecha)= " & mes & " " &
                                            "GROUP BY YEAR(fecha),MONTH(fecha)"
                    Case 2400
                        sql = "	SELECT mes, ano ,(SELECT CASE WHEN SUM(esperado) =0 THEN 0 ELSE (SUM(cartones)/SUM(esperado))*100 END) As efic  " &
                                  "FROM J_v_empaque_puntilleria v " &
                                        "WHERE anulado Is null And mes = " & mes & "  And ano = " & ano & "  " &
                                          "GROUP BY mes, ano "
                End Select
                If sql <> "" Then
