...
Public Class Frm_seg_ppto_gastos
    ...
    Public Sub MAIN(ByVal usuario As UsuarioEn)
        ...
        If centros = "" Then
            sql = "select * from F_V_centros_ppt"

            'where_sql &= "WHERE centro IN(1200,2100,2117,2200,2300,2400,2500,2600,2800,3100,3200,4100,3500,5200,6200,6400)"
        Else
            'where_sql &= "WHERE centro IN(" & centros & ")"
            sql = "select * from F_V_centros_ppt  WHERE centro IN(" & centros & ")"
        End If
    ...
    Private Function armar_dt() As DataTable
        ...
        Dim sql As String = "SELECT p.mes,p.ano,t.descripcion As tipo,c.cuenta ,c.descripcion As desc_cuenta ,(SELECT SUM (ppto) FROM J_ppto_gastos WHERE cuenta = c.cuenta  AND p.mes = mes AND ano =  p.ano " & where_centro & " ) As presupuesto ,  (SELECT SUM (valor) FROM movimiento WHERE cuenta = c.cuenta AND Year(fec) = p.ano AND MONTH(fec) = p.mes " & where_centro & " ) As saldo,(select sum(cantidad * precio_unit) from PRGPRODUCCION.dbo.J_solicitud_compra_det m join PRGPRODUCCION.dbo.J_solicitud_compra_enc l on m.numero=l.numero  where m.cuenta=c.cuenta and l.terminado is null) as ordenes_debe ,(SELECT SUM (ppto) FROM J_ppto_gastos WHERE cuenta = c.cuenta  AND ano =  p.ano " & where_centro & " ) As acum_ppto, (SELECT SUM (valor) FROM movimiento WHERE cuenta = c.cuenta AND Year(fec) = p.ano  " & where_centro & "  ) As acum_saldo " &
                             "FROM j_ppto_gastos_tipo t, J_ppto_gastos_cuentas g , cuentas c , J_ppto_gastos p    " &
                                "WHERE g.tipo = t.id And g.cuenta = c.cuenta And p.cuenta = g.cuenta And p.mes = " & mes & " And p.ano = " & ano & "" &
    ...
    Private Sub dtg_consulta_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dtg_consulta.MouseDown, dtg_ppto_aticulos.MouseDown
        ...
        If Not IsDBNull(dtg_consulta.Item("cuenta", fila_select).Value) Then
            ...
            Dim sql As String = "SELECT c.cuenta,s.centro,s.descripcion As desc_centro,sum( m.valor) as saldo  " &
                      "FROM terceros ter,j_ppto_gastos_tipo t, J_ppto_gastos_cuentas g , cuentas c , movimiento m , centros s  " &
                                  "WHERE ter.nit = m.nit AND s.centro = SUBSTRING ( cast(m.centro As varchar (25)) ,0 , 3 ) + '00' And m.cuenta = c.cuenta And g.tipo = t.id And g.cuenta = c.cuenta  And Month(m.fec) = " & mes & " And YEAR(m.fec) = " & ano & " And c.cuenta = " & cuenta & "  " & where_centro & " " &
    ...
    Private Sub ToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles ToolStripMenuItem1.Click
        If Not IsDBNull(dtg_detalle.Item("cuenta", fila_detalle).Value) Then
            ...
            Dim centro As String = dtg_detalle.Item("centro", fila_detalle).Value
            Dim subcentro As String = centro.Substring(0, 2)
            ...
            'sql = "SELECT s.centro , s.descripcion,c.descripcion As desc_cuenta,ref.codigo, ref.descripcion,SUM (lin.cantidad )As cantidad  ,SUM (lin.cantidad  * lin.costo_unitario)As saldo " &
            '            "FROM j_ppto_gastos_tipo t, J_ppto_gastos_cuentas g , cuentas c , J_ppto_gastos p , movimiento m , centros s ,documentos_lin lin , documentos doc, referencias ref " &
            '             "WHERE m.numero = doc.numero And m.tipo = doc.tipo And doc.numero = lin.numero And lin.tipo = doc.tipo And ref.codigo = lin.codigo And s.centro = m.centro And m.cuenta = c.cuenta And g.tipo = t.id And g.cuenta = c.cuenta And p.cuenta = g.cuenta And p.mes = " & mes & " And p.ano = " & ano & " And p.mes = Month(m.fec) And p.ano = Year(m.fec) And c.cuenta = " & cuenta & "  " & where_centro & " " &
            '             "GROUP BY s.centro , s.descripcion,c.descripcion ,ref.codigo, ref.descripcion"

            sql = "SELECT m.tipo,  m.numero, m.centro, ref.codigo, ref.descripcion AS Descripcion, SUM(lin.cantidad) AS cantidad, SUM(lin.cantidad * lin.costo_unitario) AS saldo, doc.centro_doc" &
                    " FROM dbo.documentos AS doc INNER JOIN " &
                         "dbo.movimiento AS m ON doc.numero = m.numero AND doc.tipo = m.tipo INNER JOIN " &
                         " dbo.documentos_lin AS lin ON doc.numero = lin.numero AND doc.tipo = lin.tipo INNER JOIN " &
                          " dbo.referencias AS ref ON lin.codigo = ref.codigo " &
                        " WHERE (MONTH(m.fec) = " & mes_detalle & ") AND (YEAR(m.fec) = " & ano_detalle & ") AND (m.cuenta = " & cuenta & ") AND (m.centro LIKE '" & subcentro & "%') " &
                        " GROUP BY m.centro, ref.codigo, ref.descripcion, doc.centro_doc, m.numero, m.tipo "
