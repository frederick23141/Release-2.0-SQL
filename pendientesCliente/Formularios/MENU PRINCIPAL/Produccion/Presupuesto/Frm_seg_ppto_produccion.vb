...
Public Class Frm_seg_ppto_produccion
    ...
    Private Function armar_dt(ByVal dias_habiles_otros As Double, ByVal dias_adic_otros As Double, ByVal dias_habiles As Double, ByVal dias_adic As Double) As DataTable
        ...
        '                               "ORDER BY g.orden"
        'Dim sql As String = "SELECT g.id_cor,g.descripcion As linea_de_produccion , g.tipo,t.descripcion As desc_tipo , SUM(p.kilos) As ppto_en_kilos , (AVG(p.kilos)/ AVG(g.conversion) ) As ppto_en_cant, p.dias_habiles AS dias_H_DC, (AVG(p.kilos)/ AVG(p.dias_habiles) ) As ppto_dia_dic " &
        '                        "FROM JJV_Grupos_seguimiento g ,PRGPRODUCCION.dbo.J_ppto_produccion p , Jjv_grupos_seguimiento_clasificacion t " &
        '                            "WHERE t.id = g.tipo AND g.id_cor = p.id_cor AND p.ano = " & ano & " AND p.mes = " & mes & " AND g.id_cor NOT IN (26,31) " &
        '                            "GROUP BY g.id_cor,g.descripcion ,g.orden, g.tipo,t.descripcion, p.dias_habiles " &
        '                               "ORDER BY g.orden"
        Dim sql As String = "SELECT  id_cor, linea_de_produccion, tipo, desc_tipo, ppto_en_kilos, dias_habiles As dias_h, ppto_dia_dic " &
                                "FROM F_V_ppto_prod " &
                                    "WHERE  ano = " & ano & " AND mes = " & mes & "" &
                                    "ORDER BY tipo ASC"

        'Dim sql As String = " SELECT Id_cor, linea_de_produccion, Tipo, desc_tipo, ppto_en_kilos, kilos, conversion, dias_habiles As dias_H_DC, ano, mes " &
        '                        "FROM F_V_ppto_prod " &
        '                            "WHERE  ano = " & ano & " AND mes = " & mes & " "
        ...
        ''LLENO LA TABLA DE CALCULOS PPTO A DIA Y PPTO EN CANTIDAD
    ...
    Private Sub calcular_ppto_a_hoy(ByRef dt As DataTable, ByVal dias_trabajados As Double, ByVal dias_habiles As Double, ByVal dias_habiles_otros As Double, ByVal dias_trabajados_otros As Double)
        ...
        For i = 0 To dt.Rows.Count - 2
            If Not IsDBNull(dt.Rows(i).Item("ppto_en_kilos")) Then
                ...
                If es_otra_area Then
                    ppto_por_dia = ppto / dias_habiles_otros
                    dt.Rows(i).Item("ppto_a_hoy") = dt.Rows(i).Item("ppto_dia_dic") * dias_trabajados_otros
                    dt.Rows(i).Item("dias_trab") = dias_trabajados_otros
                Else
                    ppto_por_dia = ppto / dias_habiles
                    dt.Rows(i).Item("ppto_a_hoy") = dt.Rows(i).Item("ppto_dia_dic") * dias_trabajados
                    dt.Rows(i).Item("dias_trab") = dias_trabajados
                End If
                dt.Rows(i).Item("ppto_por_dia") = dt.Rows(i).Item("ppto_dia_dic")
            End If
            es_otra_area = False
        Next
    End Sub
    ...
    Private Sub mostrarVariables()
        ...
        Dim ocultarDic As Boolean = True
        ...
        'prueba
        For i = 0 To dtg_presupuesto.Columns.Count - 1
            If (dtg_presupuesto.Columns(i).Name = "dias_H_DC" Or dtg_presupuesto.Columns(i).Name = "ppto_dia_dic") Then
                dtg_presupuesto.Columns(i).Visible = ocultarDic
            End If
        Next
        ...
        'prueba
        For i = 0 To dtg_ppto_area.Columns.Count - 1
            If (dtg_ppto_area.Columns(i).Name = "dias_H_DC" Or dtg_ppto_area.Columns(i).Name = "ppto_dia_dic") Then
                dtg_ppto_area.Columns(i).Visible = ocultarDic
            End If
        Next
