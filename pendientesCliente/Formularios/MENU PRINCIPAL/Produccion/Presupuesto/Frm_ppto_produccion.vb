...
Public Class Frm_ppto_produccion
    ...
    Private Sub totalizarDtg()
        ...
        For j = 0 To dtg_presupuesto.Columns.Count - 1
            'MessageBox.Show("columna " & " : " & j & dtg_presupuesto.Columns(j).Name, "ciclo 1!", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            If (nom_col = "kilos" Or nom_col = "cantidad") Then
                For i = 0 To dtg_presupuesto.Rows.Count - 2
                    If Not IsDBNull(dtg_presupuesto.Item(j, i).Value) Then
                        ...
                        'MessageBox.Show("se sumo " & dtg_presupuesto.Item(j, i).Value, "ciclo 2!", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    End If
                Next

                dtg_presupuesto.Item(j, dtg_presupuesto.Rows.Count - 1).Value = sum
                'MessageBox.Show("EN columna quedo final " & dtg_presupuesto.Item(j, dtg_presupuesto.Rows.Count - 1).Value, "dato final!", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                sum = 0
            End If
        Next
    End Sub
