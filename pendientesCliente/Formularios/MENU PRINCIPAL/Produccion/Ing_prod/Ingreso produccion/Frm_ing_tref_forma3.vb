    ...
    Private Sub dtg_planilla_CellValueChanged(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dtg_planilla.CellValueChanged
        If (carga_comp) Then
            ...
            ElseIf (col = "col_efic") Then
                ...
                End If
            ElseIf (col = "txt_p0" Or col = "txt_p1" Or col = "txt_p2" Or col = "txt_p3" Or col = "txt_p4" Or col = "txt_p5" Or col = "txt_p6" Or col = "txt_p7" Or col = "txt_p8" Or col = "txt_p9" Or col = "txt_p10" Or col = "txt_p11" Or col = "txt_p12" Or col = "txt_p13" Or col = "txt_p14") Then
                dtg_planilla.Item(col, fila).Value = objopSimplesLn.operacionSumaDtg(dtg_planilla.Item(col, fila).Value)
                calc_T_sin_just(fila)
    ...
    Private Sub guardar()
        ...
        If (id_modificar <> 0) Then
            ...
            If (consec_ppal <> 0) Then
                sql_gral = "INSERT INTO J_prod_tref_completo " &
                                   "(consecutivo,id_detalle,nit,fecha,turno,notas,maquina,cod_destino,h_ini,h_fin,diametro,alambron,reproceso,und_ini,und_fin,velocidad,Min_trab " &
                                   ",paro0,Paro1,paro2,paro3,paro4,paro5,paro6,paro7,paro8,paro9,paro10,paro11,paro12,paro13,paro14) " &
        ...
        If (verificar_planilla(dtg_planilla)) Then
            For i = 0 To dtg_planilla.RowCount - 1
                ...
                paro14 = dtg_planilla("txt_p14", i).Value
                If ((maquina <> 0 And calibre <> 0 And destino <> 0 And min_trab <> 0)) Then
                    sql_total = sql_gral & maquina & "," & destino & ",'" & hora_ini & "','" & hora_fin & "'," & calibre & "," & alamb & "," & rep & "," & und_ini & "," & und_fin & "," & velocidad & "," & min_trab & "," &
                        paro0 & "," & paro1 & "," & paro2 & "," & paro3 & "," & paro4 & "," & paro5 & "," & paro6 & "," & paro7 & "," & paro8 & "," & paro9 & "," & paro10 & "," & paro11 & "," & paro12 & "," & paro13 & "," & paro14 & ")"
