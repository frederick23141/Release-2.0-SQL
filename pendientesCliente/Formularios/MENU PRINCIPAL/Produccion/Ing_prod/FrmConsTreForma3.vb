...
Public Class FrmConsTreForma3
    ...
    Public Sub cargar_consulta()
        ...
        dtg_consulta.Columns("paro0").HeaderText = "Aseo"
        dtg_consulta.Columns("paro1").HeaderText = "Cambio de programa"
        dtg_consulta.Columns("paro2").HeaderText = "Reviente"
        dtg_consulta.Columns("paro3").HeaderText = "Cambio de hilera"
        dtg_consulta.Columns("paro4").HeaderText = "Soldar MP"
        dtg_consulta.Columns("paro5").HeaderText = "Enredos"
        dtg_consulta.Columns("paro6").HeaderText = "Daño Electrico"
        dtg_consulta.Columns("paro7").HeaderText = "Daño Mecánico"
        dtg_consulta.Columns("paro8").HeaderText = "Otros"
        dtg_consulta.Columns("paro9").HeaderText = "Alimentación"
        dtg_consulta.Columns("paro10").HeaderText = "Probelmas materia prima"
        dtg_consulta.Columns("paro11").HeaderText = "Abastecimiento"
        dtg_consulta.Columns("paro12").HeaderText = "Capacitación Y/O Reunión"
        dtg_consulta.Columns("paro13").HeaderText = "Mantenimiento programado"
        dtg_consulta.Columns("paro14").HeaderText = "Alist. Inicio Turno"
        dtg_consulta.Columns("paro15").HeaderText = "Ausencia Operario"
    ...
    Private Sub cargar_paros()
        ...
        sql = "SELECT  SUM (paro0)As paro0,SUM (paro1 )As paro1,SUM (paro2 )As paro2,SUM (paro3 )As paro3,SUM (paro4 )As paro4,SUM (paro5 )As paro5,SUM (paro6 )As paro6,SUM (paro7 )As paro7, " &
                                     "SUM(paro8)As paro8 , SUM(paro9)As paro9, SUM(paro10) As paro10, SUM(paro11) As paro11, SUM(paro12) As paro12, SUM(paro13) As paro13,SUM(paro14) As paro14,SUM(paro15) As paro15 " &
                                         "FROM j_v_tref_completo  " &
                                            where_fec & " " & whereMaquina & " " & whereOperario
        Dim vec() As Object = obj_Ing_prodLn.vec_paros(sql, 15)
        Chart1.Visible = True
        Chart1.Titles.Clear()
        Chart1.Series.Clear()
    ...
    Private Function desc_paro(ByVal numParo As Integer) As String
        ...
        Select Case numParo
            ...
            Case 14
                resp = "ALISTAMIENTO INICIO DEL TURNO"
            Case 15
                resp = "AUSENCIA DE OPERARIO"
        End Select
        Return resp & "(" & numParo & ")"
    End Function
    ...
    Private Sub CheckBox1_CheckedChanged(sender As Object, e As EventArgs) Handles CheckBox1.CheckedChanged
        If (CheckBox1.Checked = True) Then
            dtg_consulta.Columns("velocidad").Visible = True
            dtg_consulta.Columns("h_ini").Visible = True
            dtg_consulta.Columns("h_fin").Visible = True
            dtg_consulta.Columns("und_ini").Visible = True
            dtg_consulta.Columns("und_fin").Visible = True
            dtg_consulta.Columns("destino").Visible = True
            dtg_consulta.Columns("anulado").Visible = True
            For i = 0 To dtg_consulta.Rows.Count - 1
                If Not IsDBNull(dtg_consulta.Item("h_ini", i).Value) Then
                    dtg_consulta.Item("h_ini", i).Style.BackColor = Color.Yellow
                End If
                If Not IsDBNull(dtg_consulta.Item("h_fin", i).Value) Then
                    dtg_consulta.Item("h_fin", i).Style.BackColor = Color.Yellow
                End If
            Next
        Else
            dtg_consulta.Columns("velocidad").Visible = False
