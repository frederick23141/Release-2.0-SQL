...
Public Class Frm_Ing_puntilleria
    ...
    Public Sub New()
        ...
        AddHandler MyBase.Load, New EventHandler(AddressOf Me.Ing_puntilleria_Load)
        Me.obj_Ing_prodLn = New Ing_prodLn
        Me.carga_comp = False
        Me.h_prog = 0
        Me.operario = 0
        Me.obj_opSimplesLn = New Op_simpesLn
    ...
    Private Sub Ing_puntilleria_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ''cbo_operario.SelectedValue = nit
        ''cbo_operario.SelectedValue = 
        'cargar_cbo_dtg()
        'carga_comp = True


        Dim cadena As String = "select Codigo,Descripcion from J_turnos  "
        Dim column As New DataColumn
        Me.cbo_fecha.Value = DateAndTime.Now.AddDays(-1)
        Me.cbo_turno.DataSource = Me.obj_Ing_prodLn.listar_datatable(cadena, "PRODUCCION")
        Me.cbo_turno.ValueMember = "Codigo"
        Me.cbo_turno.DisplayMember = "Descripcion"
        Me.cbo_turno.Text = "Seleccione"
        cadena = "Select nit,nombres  from V_nom_personal_Activo_con_maquila WHERE (centro_planta in (2300,6400) or centro_planta is null) ORDER BY nombres  "
        Me.cbo_operario.DataSource = Me.obj_Ing_prodLn.listar_datatable(cadena, "CORSAN")
        Me.cbo_operario.ValueMember = "nit"
        Me.cbo_operario.DisplayMember = "nombres"
    ...
    Private Sub cargar_cbo_dtg()
        ...
        'Dim dt As DataTable
        'Dim row As DataRow
        'sql = "select  codigoM,nombre   from j_Maquinas where TipoMaquina in (2,3) AND activa = 1  ORDER BY Nombre"
        ...
        cadena = "select  codigoM,nombre   from j_Maquinas where TipoMaquina in (2,3) AND activa = 1 order by nombre"
        Dim table As DataTable = Me.obj_Ing_prodLn.listar_datatable(cadena, "PRODUCCION")
        Dim row As DataRow = table.NewRow
        row("codigoM") = 0
        row("nombre") = ""
        table.Rows.Add(row)
        Me.id_maquina.DataSource = table
        Me.id_maquina.ValueMember = "codigoM"
        Me.id_maquina.DisplayMember = "nombre"
        cadena = "select  id_ref,descripcion   from J_referencias where tipo in (2,3) "
        table = Me.obj_Ing_prodLn.listar_datatable(cadena, "PRODUCCION")
        row = table.NewRow
        row("id_ref") = 0
        row("descripcion") = ""
        table.Rows.Add(row)
        Me.id_producto.DataSource = table
        Me.id_producto.ValueMember = "id_ref"
        Me.id_producto.DisplayMember = "descripcion"
        Dim num As Integer = 0
        Do
            Me.dtg_planilla.Rows.Add()
            num += 1
        Loop While (num <= 7)
    ...
    Public Function verificar_planilla(ByVal dtg As DataGridView) As Boolean
        ...
        'Dim h_trab = (dtg_planilla.Item("txt_uni_fin", i).Value - dtg_planilla.Item("txt_un_ini", i).Value)
        Dim referencia As String = ""
        For i = 0 To dtg_planilla.RowCount - 1
            maquina = dtg_planilla.Item("id_maquina", i).Value
            referencia = dtg_planilla("id_producto", i).Value
        ...
        'Dim str As String = ""
        'Dim num4 As Integer = (Me.dtg_planilla.RowCount - 1)
        'Dim i As Integer = 0
        'Do While (i <= num4)
        '    num = dtg_planilla("id_maquina", i).Value
        '    str = dtg_planilla("id_producto", i).Value
        '    num2 = dtg_planilla("txt_un_ini", i).Value
        '    num3 = dtg_planilla("txt_uni_fin", i).Value
        '    Me.dtg_planilla.Rows(i).DefaultCellStyle.BackColor = Color.Red
        '    If (num2 > num3) Then
        '        MessageBox.Show("El hodometro no puede ser negativo! ", "Verifique", MessageBoxButtons.OK, MessageBoxIcon.Hand)
        '        Me.dtg_planilla("txt_uni_fin", i).Value = 0
        '        Me.dtg_planilla("txt_un_ini", i).Value = 0
        '        flag2 = False
        '        i = (Me.dtg_planilla.RowCount - 1)
        '    ElseIf (((((num <> 0) Or (str <> Nothing)) Or Not (num2 = 0)) Or Not (num3 = 0)) AndAlso Not ((((num <> 0) And (str <> Nothing)) And Not (num2 = 0)) And Not (num3 = 0))) Then
        '        flag2 = False
        '        i = (Me.dtg_planilla.RowCount - 1)
        '    End If
        '    i += 1
    ...
    Private Function paros_ant() As Double
        ...
        Dim sql As String = "SELECT SUM (paro1 )+SUM (paro2 )+SUM (paro3 )+SUM (paro4 )+SUM (paro5 )+SUM (paro6 )+SUM (paro7 )+ " &
                                    "SUM(paro8) + SUM(paro9) + SUM(paro10) " &
