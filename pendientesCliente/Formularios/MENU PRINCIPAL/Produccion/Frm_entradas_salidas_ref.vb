...
Public Class Frm_entradas_salidas_ref
    ...
    Private Sub cargar_consulta()
        ...
        If (chk_consol_linea.Checked) Then
            ...
            End If
        Else
            ''prueba   dentro del check funcion para los kilos 
            If (ver_kilos.Checked) Then
                selectSql &= "s.codigo, r.descripcion, (s.can_ini * r.conversion) AS can_ini, (s.can_ent * r.conversion) As cant_ent, (s.can_sal * r.conversion) AS can_sal, (((s.can_ini * r.conversion) + (s.can_ent * r.conversion)) - (s.can_sal * r.conversion))As stock, cos_ini, cos_ent, cos_sal, ((s.cos_ini + s.cos_ent) - s.cos_sal)As costo_stock , s.mes, s.ano "
            Else
            ...
            'selectSql &= "s.codigo, r.descripcion, (s.can_ini * r.conversion) As cant_ini, (s.can_ent * r.conversion) As cant_ent, (s.can_sal * r.conversion) As cant_sal, ((s.can_ini * r.conversion) + (s.can_ent * r.conversion) - (s.can_sal * r.conversion)) As stock, s.cos_ini, s.cos_ent, s.cos_sal, ((s.cos_ini + s.cos_ent) - s.cos_sal) As costo_stock, s.mes, s.ano"
