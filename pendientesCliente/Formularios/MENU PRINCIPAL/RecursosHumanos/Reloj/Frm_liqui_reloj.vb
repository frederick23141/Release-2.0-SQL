...
Public Class Frm_liqui_reloj
    ...
    Public Sub MAIN(ByVal nomb_modulo As String, ByVal usuario As UsuarioEn, ByVal es_validacion_de_inicio As Boolean)
        ...
        End If
        If centros = "" Then
            centros = "1100,1200,2100,2117,2200,2300,2400,2500,2600,2800,3100,3200,4100,4110,4500,3500,5200,5300,5400,6200,6400, 4200,4100,4300"
        End If
        ...
        Dim dt_conceptos As DataTable = objOpSimplesLn.listar_datatable("SELECT concepto FROM J_permiso_novedad WHERE usuario = '" & usuario.usuario.Trim & "' ", "CONTROL")
        'Dim dt_conceptos As DataTable = objOpSimplesLn.listar_datatable("SELECT concepto FROM J_permiso_novedad WHERE usuario = '" & usuario.usuario.Trim & "' ", "CONTROL")
        'en el metodo anterior eliminamos los conceptos a ignorar(34,36,77) . para poderlos enviar a y_suspenciones y a y_novedades 
    ...
    ''' ESTA FUNCION INTENTAMOS RECREAR LA CANTIDAD DE TURNOS Y TODOS CON SUS EXTRAS DE FORMA CORRECTA
    Private Function procesar_marcaciones_original(ByVal fecIni As String, ByVal fecFin As String, ByVal where_centro As String, ByVal where_operario As String, ByVal sqlOrder As String, ByVal where_turno As String) As DataTable
        Dim sql As String = "SELECT CAST(r.Consecutivo As varchar)As Consec,CAST(r.nit As varchar) As nit,p.nombres,p.oficio,p.contrato,y.turno,y.planta,y.basico_mes,CAST(p.centro As varchar) As centro,y.basico_hora,r.FechaEntrada,r.FechaSalida,c.sabado,c.domingo,c.festivo,SUM(f.extras )As ExtP ,SUM(t.ord_diur + t.ord_noct ) As OrdP,r.notas,f.id As id_enc , (SELECT SUM (horas) FROM R_compromiso_horas_a_pagar WHERE Nit_Trabajador= r.nit AND Activo = 'True' AND horas >0 AND concepto_90 is null AND concepto_84 is null)As debe " &
        ...
        If (dt.Rows.Count - 1 <> -1) Then
            ...
            progBar.Minimum = 0
            GunaCircleProgressBar.Maximum = dt.Rows.Count - 1
            GunaCircleProgressBar.Minimum = 0
        End If

        For i = 0 To dt.Rows.Count - 1
            GunaCircleProgressBar.Visible = True

            progBar.PerformStep()
            progBar.Refresh()
            GunaCircleProgressBar.Increment(1)
            GunaCircleProgressBar.Refresh()
            ...
            Dim dia_Semana As Integer = Weekday(fec_hora_entro, vbMonday)
            MessageBox.Show("el dia procesado es : " & dia_Semana, "dia semana", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            If ((objRelojLn.verificar_turno_periodo(dt.Rows(i).Item("nit"), id_periodo, 104, fec_hora_entro) Or
                                  objRelojLn.verificar_turno_periodo(dt.Rows(i).Item("nit"), id_periodo, 105, fec_hora_entro) Or
                                  objRelojLn.verificar_turno_periodo(dt.Rows(i).Item("nit"), id_periodo, 106, fec_hora_entro) Or
                                      objRelojLn.verificar_turno_periodo(dt.Rows(i).Item("nit"), id_periodo, 107, fec_hora_entro)) And dt.Rows(i).Item("sabado") = "True") Then
            ...
            Else
                If dia_festivo Then
                    If ((objRelojLn.verificar_turno_periodo(dt.Rows(i).Item("nit"), id_periodo, 1, fec_hora_entro) Or
                                 ...
                                 objRelojLn.verificar_turno_periodo(dt.Rows(i).Item("nit"), id_periodo, 4, fec_hora_entro) Or
                                     ...
                                     objRelojLn.verificar_turno_periodo(dt.Rows(i).Item("nit"), id_periodo, 6, fec_hora_entro) Or
                                       objRelojLn.verificar_turno_periodo(dt.Rows(i).Item("nit"), id_periodo, 7, fec_hora_entro))) Then
                        dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("ExtP") += dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("OrdP")
                        dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("OrdP") = 0
                    End If
                End If
            ...
            fec_hora_entro = fec_hora_entro.AddSeconds(-fec_hora_entro.Second)


            fec_hora_salio = fec_hora_salio.AddMinutes(-fec_hora_salio.Minute + 1)
                ...
                If (min_dif >= 50) Then
                    ...
                    If (fec_hora_entro.TimeOfDay >= TimeSpan.Parse("05:30:00") And fec_hora_entro.TimeOfDay <= TimeSpan.Parse("20:30:00")) Then
                        If (dt.Rows(i).Item("domingo") = "True" Or dt.Rows(i).Item("festivo") = "True") Then
                            ...
                            Else
                                If IsDBNull(dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("Dcomp")) Then
                                    dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("Dcomp") = horas_add / 2
                                Else
                                    dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("Dcomp") += horas_add / 2
                                End If
                        ...
                        ElseIf (dt.Rows(i).Item("sabado") = "True") Then
                            ...
                            Else
                                ''' PARA DESCANSOS COMPENSATORIOS
                                If IsDBNull(dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("DComp")) Then

                                    dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("DComp") = horas_add
                                Else
                                    dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("DComp") += horas_add
                                End If
                            End If
                    ...
                    Else
                        ...
                        If (dia_festivo) Then
                            If (objRelojLn.verificar_turno_dia(dt.Rows(i).Item("nit"), objOpSimplesLn.cambiarFormatoFecha(dt.Rows(i).Item("FechaEntrada")), 3) Or
                               ...
                               objRelojLn.verificar_turno_dia(dt.Rows(i).Item("nit"), objOpSimplesLn.cambiarFormatoFecha(dt.Rows(i).Item("FechaEntrada")), 99)) Then
...
#Enable Warning BC42303 ' El comentario XML no puede aparecer en un método o una propiedad. Se omitirá el comentario XML.
                        ...
                        ElseIf dia_festivo = True Then
                            ...
                            If (fec_hora_entro.TimeOfDay >= TimeSpan.Parse("00:00:01") And fec_hora_entro.TimeOfDay <= TimeSpan.Parse("06:00:00")) Then
                                If IsDBNull(dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("DN")) Then
                                    dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("DN") = horas_add
                                Else
                                    dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("DN") += horas_add
                        ...
                        ElseIf dt.Rows(i).Item("sabado") = "True" Then
                            ...
                            ''''prueba para rn sabado


                            If (fec_hora_entro.TimeOfDay <= TimeSpan.Parse("23:59:00") And Not dia_compensatorio) Then
                                If IsDBNull(dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("RN")) Then
                                    dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("RN") = horas_add
                                Else
                                    dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("RN") += horas_add
                            ...
                            If objRelojLn.verificar_turno_dia(dt.Rows(i).Item("nit"), objOpSimplesLn.cambiarFormatoFecha(dt.Rows(i).Item("FechaEntrada")), 99) Then
#Disable Warning BC42303 ' El comentario XML no puede aparecer en un método o una propiedad. Se omitirá el comentario XML.
                                ''' PARA DESCANSOS COMPENSATORIOS
                                If IsDBNull(dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("DComp")) Then
#Enable Warning BC42303 ' El comentario XML no puede aparecer en un método o una propiedad. Se omitirá el comentario XML.
                        ...
                        ElseIf (dt.Rows(i).Item("domingo") = "True" Or dt.Rows(i).Item("festivo") = "True") Then
                            ...
                            If Not dia_compensatorio Then
                                If (objRelojLn.verificar_turno_dia(dt.Rows(i).Item("nit"), objOpSimplesLn.cambiarFormatoFecha(dt.Rows(i).Item("FechaEntrada")), 3) Or
                                                                  objRelojLn.verificar_turno_dia(dt.Rows(i).Item("nit"), objOpSimplesLn.cambiarFormatoFecha(dt.Rows(i).Item("FechaEntrada")), 9)) Then
                        ...
                        Else
                            ...
                            Else
                                ...
                                Else
                                    'If (dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("RN") < ord_prog And horas_ord < ord_prog) Then
                                    If (dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("RN") < ord_prog) Then
                                        dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("RN") += horas_add
                                        horas_ord += horas_add
                                        If IsDBNull(dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("OrD")) Then
                                            dt_procesado.Rows(dt_procesado.Rows.Count - 1).Item("OrD") = horas_add
            ...
            horas_ord = 0
        Next
        GunaCircleProgressBar.Reset()
        GunaCircleProgressBar.Hide()
        dia_compensatorio = False
        dt_procesado.Rows.Add()
        Return dt_procesado
    ...
    ''' ESTA FUNCION INTENTAMOS CREAR UN RELOJ GENERICO CON LOS NUEVOS CONCEPTO
    ...
    Private Function procesar_marcaciones_historico(ByVal fecIni As String, ByVal fecFin As String, ByVal where_centro As String, ByVal where_operario As String, ByVal sqlOrder As String, ByVal where_turno As String) As DataTable
        Dim sql As String = "SELECT CAST(r.Consecutivo As varchar)As Consec,CAST(r.nit As varchar) As nit,p.nombres,p.oficio,p.contrato,y.turno,y.planta,y.basico_mes,CAST(p.centro As varchar) As centro,y.basico_hora,r.FechaEntrada,r.FechaSalida,c.sabado,c.domingo,c.festivo,SUM(f.extras )As ExtP ,SUM(t.ord_diur + t.ord_noct ) As OrdP,r.notas,f.id As id_enc , (SELECT SUM (horas) FROM R_compromiso_horas_a_pagar WHERE Nit_Trabajador= r.nit AND Activo = 'True' AND horas >0 AND concepto_90 is null AND concepto_84 is null)As debe " &
                        "FROM r_personal_registros r , CORSAN.dbo.y_calendario c,CORSAN.dbo.V_nom_personal_activo p ,CORSAN.dbo.y_personal_contratos y , J_v_operario_fecha_turno f, CORSAN.dbo.J_turnos t  " &
                    "WHERE t.id = f.id_turno AND p.contrato = y.contrato  AND p.nit = r.nit AND f.nit = r.nit AND CAST (r.FechaEntrada AS date ) = f.fecha AND CAST (r.FechaEntrada AS date ) = c.fecha AND FechaSalida is not null AND CAST(r.FechaEntrada AS date )  Between '" & fecIni & "' AND '" & fecFin & "  23:59:59' " & where_centro & where_operario & where_turno & " " &
                        "GROUP BY Consecutivo, r.nit ,p.nombres,p.oficio,p.centro,p.contrato,y.turno,y.planta,y.basico_hora,y.basico_mes ,r.FechaEntrada,r.FechaSalida,c.sabado,c.domingo,c.festivo,r.notas,f.id " &
...
#Enable Warning BC42303 ' El comentario XML no puede aparecer en un método o una propiedad. Se omitirá el comentario XML.
    ...
    Private Sub consolidarOriginal(ByVal fecIni As String, ByVal fecFin As String, ByVal where_centro As String, ByVal where_operario As String, ByVal where_turno As String, ByVal dt As DataTable)
        ...
        dt_consolidado.Columns.Add("PL", GetType(Integer)) '------------------concepto =38
        dt_consolidado.Columns.Add("PRH", GetType(Integer)) '------------------concepto =77
        dt_consolidado.Columns.Add("HL", GetType(Integer)) '------------------concepto =90
        dt_consolidado.Columns.Add("PEM", GetType(Integer)) '------------------concepto =91
        dt_consolidado.Columns.Add("RDOMF", GetType(Integer)) '------------------concepto =95
        dt_consolidado.Columns.Add("PN", GetType(Integer)) '------------------concepto =557 (ANTERIOR 510) 



        'YSUSPENCIONES
        dt_consolidado.Columns.Add("PLICR", GetType(Integer)) '------------------concepto =8
        dt_consolidado.Columns.Add("PR", GetType(Integer)) '------------------concepto =34
        dt_consolidado.Columns.Add("D FMLIA", GetType(Integer)) '------------------concepto =88
        dt_consolidado.Columns.Add("DS", GetType(Integer)) '------------------concepto =511
        dt_consolidado.Columns.Add("SU", GetType(Integer)) '------------------concepto =512
        dt_consolidado.Columns.Add("LICNR", GetType(Integer)) '------------------concepto =513
        ...
        For i = 0 To dt_datos.Rows.Count - 2
            ...
            If (fila = -1) Then
                ...
                dt_tiempo_ordinario = objRelojLn.calcular_horas_ordinarias(id_periodo, dt_datos.Rows(i).Item("nit"), horas_laborales, horas_totales_periodo, fecFin, fecIni, dt_datos)
                For k = 0 To dt_tiempo_ordinario.Rows.Count - 1
                    ...
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("OrD") = horas_totales_periodo
                    ...
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("PL") = dt_tiempo_ordinario.Rows(k).Item("PL")
                    ...
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("ED") = dt_tiempo_ordinario.Rows(k).Item("ED")
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("EN") = dt_tiempo_ordinario.Rows(k).Item("EN")
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("PEM") = dt_tiempo_ordinario.Rows(k).Item("PEM")
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("RDOMF") = dt_tiempo_ordinario.Rows(k).Item("RDOMF")
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("PLICR") = dt_tiempo_ordinario.Rows(k).Item("PLICR")
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("D FMLIA") = dt_tiempo_ordinario.Rows(k).Item("D FMLIA")
                    dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("LICNR") = dt_tiempo_ordinario.Rows(k).Item("LICNR")
        ...
        'ESTA LINEA CALCULA LAS HORAS ORDINARIAS
        ...
        For i = 0 To dt_datos.Rows.Count - 2
            ...
            If (fila = -1) Then
                ...
                If IsNumeric(dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("Dcomp")) Then
                    ...
                    End If
                End If
                If IsNumeric(dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("PV")) Then
                    If Not IsDBNull(dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("HR")) Then
                        dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("HR") += dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("PV")
                    Else
                        dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("HR") = dt_consolidado.Rows(dt_consolidado.Rows.Count - 1).Item("PV")
                    End If
                End If
    ...
    '                If dtg_consulta.Columns(j).Name = "OrD" Or dtg_consulta.Columns(j).Name = "HL" Or dtg_consulta.Columns(j).Name = "DComp" Or dtg_consulta.Columns(j).Name = "PL" Or
    '                     dtg_consulta.Columns(j).Name = "DF" Or dtg_consulta.Columns(j).Name = "PRH" Or dtg_consulta.Columns(j).Name = "PR" Or dtg_consulta.Columns(j).Name = "PD" Or
    '                    dtg_consulta.Columns(j).Name = "PE" Or dtg_consulta.Columns(j).Name = "PC" Or dtg_consulta.Columns(j).Name = "PN" Or
    '                    dtg_consulta.Columns(j).Name = "PEM" Or dtg_consulta.Columns(j).Name = "RDOMF" Or dtg_consulta.Columns(j).Name = "PLICR" Or
    '                    dtg_consulta.Columns(j).Name = "D FMLIA" Or dtg_consulta.Columns(j).Name = "LICNR" Or
    ...
    '                        Or dtg_consulta.Columns(j).Name = "SU" Or dtg_consulta.Columns(j).Name = "DS" Or dtg_consulta.Columns(j).Name = "FN" Then
    '#Enable Warning BC42303 ' El comentario XML no puede aparecer en un método o una propiedad. Se omitirá el comentario XML.
    ...
    Private Sub tot_ordinarias()
        ...
        For i = 2 To dtg_consulta.Rows.Count - 1
            For j = 0 To dtg_consulta.Columns.Count - 1
                ...
                '    dtg_consulta.Columns(j).Name = "D FMLIA" Or dtg_consulta.Columns(j).Name = "LICNR" Or
                '    dtg_consulta.Columns(j).Name = "IR" Or dtg_consulta.Columns(j).Name = "IA" Or dtg_consulta.Columns(j).Name = "LM" Or dtg_consulta.Columns(j).Name = "V" _
                '        Or dtg_consulta.Columns(j).Name = "SU" Or dtg_consulta.Columns(j).Name = "DS" Or dtg_consulta.Columns(j).Name = "FN" Or dtg_consulta.Columns(j).Name = "PV" Then
                If dtg_consulta.Columns(j).Name = "OrD" Or dtg_consulta.Columns(j).Name = "HL" Or dtg_consulta.Columns(j).Name = "DComp" Or dtg_consulta.Columns(j).Name = "PL" Or
                     dtg_consulta.Columns(j).Name = "DF" Or dtg_consulta.Columns(j).Name = "PRH" Or dtg_consulta.Columns(j).Name = "PR" Or dtg_consulta.Columns(j).Name = "PD" Or
                    dtg_consulta.Columns(j).Name = "PE" Or dtg_consulta.Columns(j).Name = "PC" Or dtg_consulta.Columns(j).Name = "PN" Or
                    dtg_consulta.Columns(j).Name = "PEM" Or dtg_consulta.Columns(j).Name = "PLICR" Or
                    dtg_consulta.Columns(j).Name = "D FMLIA" Or dtg_consulta.Columns(j).Name = "LICNR" Or
                    dtg_consulta.Columns(j).Name = "IR" Or dtg_consulta.Columns(j).Name = "IA" Or dtg_consulta.Columns(j).Name = "LM" Or dtg_consulta.Columns(j).Name = "V" _
                        Or dtg_consulta.Columns(j).Name = "SU" Or dtg_consulta.Columns(j).Name = "DS" Or dtg_consulta.Columns(j).Name = "FN" Or dtg_consulta.Columns(j).Name = "PV" Then
#Enable Warning BC42303 ' El comentario XML no puede aparecer en un método o una propiedad. Se omitirá el comentario XML.
        ...
        For i = 2 To dtg_consulta.Rows.Count - 1
            If IsNumeric(dtg_consulta.Item("TotOrD", i).Value) Then
                If (dtg_consulta.Item("TotOrD", i).Value > horas_totales_periodo) Then
                    ...
                    'For j = 0 To dtg_consulta.Columns.Count - 1
                    '    If dtg_consulta.Columns(j).Name = "PRH" Or dtg_consulta.Columns(j).Name = "HL" Or dtg_consulta.Columns(j).Name = "PR" Or dtg_consulta.Columns(j).Name = "PD" Or
                    '        dtg_consulta.Columns(j).Name = "PE" Or dtg_consulta.Columns(j).Name = "PC" Or dtg_consulta.Columns(j).Name = "PN" Or dtg_consulta.Columns(j).Name = "V" Or dtg_consulta.Columns(j).Name = "SU" Or dtg_consulta.Columns(j).Name = "PL" Then
                    '        If IsNumeric(dtg_consulta.Item(j, i).Value) Then
                    '            tot_horas_nov_permisos += dtg_consulta.Item(j, i).Value
                    '        End If
                    '    End If
                    ...
                    For j = 0 To dtg_consulta.Columns.Count - 1
                        If dtg_consulta.Columns(j).Name = "PRH" Or dtg_consulta.Columns(j).Name = "HL" Or dtg_consulta.Columns(j).Name = "PR" Or dtg_consulta.Columns(j).Name = "PD" Or
                            dtg_consulta.Columns(j).Name = "PE" Or dtg_consulta.Columns(j).Name = "PV" Or dtg_consulta.Columns(j).Name = "PC" Or dtg_consulta.Columns(j).Name = "PN" Or dtg_consulta.Columns(j).Name = "V" Or dtg_consulta.Columns(j).Name = "SU" Or dtg_consulta.Columns(j).Name = "PL" Then
                            If IsNumeric(dtg_consulta.Item(j, i).Value) Then
                                tot_horas_nov_permisos += dtg_consulta.Item(j, i).Value
                            End If
                        End If
    ...
    Private Sub addConceptos(ByRef dt As DataTable)
        ...
        dt.Rows(dt.Rows.Count - 1).Item("PN") = 557
        ...
        dt.Rows(dt.Rows.Count - 1).Item("PL") = 38
        dt.Rows(dt.Rows.Count - 1).Item("PLICR") = 8
        dt.Rows(dt.Rows.Count - 1).Item("PEM") = 91
        dt.Rows(dt.Rows.Count - 1).Item("RDOMF") = 95
        dt.Rows(dt.Rows.Count - 1).Item("D FMLIA") = 88
        dt.Rows(dt.Rows.Count - 1).Item("LICNR") = 513
        dt.Rows(dt.Rows.Count - 1).Item("FN") = 516
        ...
        ''conceptos_liquidar = "1,30,31,32,2,13,14,15,16,33,34,35,36,37,510,5,6,7,512,511,10,17,77,90"
        conceptos_liquidar = "1,17,31,32,2,13,14,15,16,33,34,77,35,36,37,38,557,90,5,6,7,512,511,38,8,91,95,88,513,516"
        ...
        For i = 0 To dt_porc_conceptos.Rows.Count - 1
            Select Case dt_porc_conceptos.Rows(i).Item("concepto")
                ...
                Case 38
                    dt.Rows(dt.Rows.Count - 1).Item("PL") = 0
            End Select
        Next
    End Sub
    Private Sub consolidar_marcacion(ByVal fila_datos As Integer, ByVal fila_consolidado As Integer, ByRef dt_consolidado As DataTable, ByRef dt_datos As DataTable)
        ...
        For j = 0 To dt_datos.Columns.Count - 1
            ...
            If (dt_datos.Columns(j).ColumnName = "ED" Or dt_datos.Columns(j).ColumnName = "EN" Or dt_datos.Columns(j).ColumnName = "EDF" Or
                        dt_datos.Columns(j).ColumnName = "ENF" Or dt_datos.Columns(j).ColumnName = "OrD") Then
    ...
    Private Sub consolidar_marcacion_todo_Extra(ByVal fila_datos As Integer, ByVal fila_consolidado As Integer, ByRef dt_consolidado As DataTable, ByVal tot_horas_dia As Integer, ByRef dt_datos As DataTable)
        ...
        For j = 0 To dt_datos.Columns.Count - 1
            ...
            If (dt_datos.Columns(j).ColumnName = "ED" Or dt_datos.Columns(j).ColumnName = "EN" Or
            dt_datos.Columns(j).ColumnName = "EDF" Or dt_datos.Columns(j).ColumnName = "ENF" Or
                    dt_datos.Columns(j).ColumnName = "OrD" Or dt_datos.Columns(j).ColumnName = "RN") Then
                ...
                'If (dt_datos.Columns(j).ColumnName = "DD" Or dt_datos.Columns(j).ColumnName = "DN" Or
                If (dt_datos.Columns(j).ColumnName = "ED" Or dt_datos.Columns(j).ColumnName = "EDF" Or
    ...
    Private Sub DetalleToolStripMenuItem_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If (dtg_consulta.Columns(colSelect_consulta).Name = "PRH" Or dtg_consulta.Columns(colSelect_consulta).Name = "PR" Or dtg_consulta.Columns(colSelect_consulta).Name = "PD" Or dtg_consulta.Columns(colSelect_consulta).Name = "PE" Or dtg_consulta.Columns(colSelect_consulta).Name = "PC" _
        ...
        ElseIf (dtg_consulta.Columns(colSelect_consulta).Name = "SU" Or dtg_consulta.Columns(colSelect_consulta).Name = "DS" Or dtg_consulta.Columns(colSelect_consulta).Name = "LICNR") Then
    ...
    Private Sub detalle_suspensiones(ByVal nit As Double)
        Dim sql As String = "SELECT s.nit,s.fecha_inicial,s.fecha_final,s.concepto ,c.descripcion    " &
                                "FROM y_suspensiones s, y_conceptos_nom c   " &
                                     "WHERE s.concepto IN (511,512,513) AND s.concepto = c.concepto AND (s.fecha_inicial >= '" & objOpSimplesLn.cambiarFormatoFecha(dFecIni_periodo) & "' OR s.fecha_final >= '" & objOpSimplesLn.cambiarFormatoFecha(dFecFin_periodo) & "') AND  s.nit = " & nit & " "
        dtg_detalle.DataSource = objOpSimplesLn.listar_datatable(sql, "CORSAN")
        dtg_detalle.Columns("fecha_inicial").DefaultCellStyle.Format = "d"
        dtg_detalle.Columns("fecha_final").DefaultCellStyle.Format = "d"
    End Sub
    ...
    Private Sub detalle_turno_prog(ByVal nit As Double)
        ...
        Dim sql As String = "SELECT e.id As id_enc,e.nit,e.periodo , d.fecha,CAST(d.turno as varchar) + ' - ' + t.Descripcion As turno  " &
                                    "FROM J_prog_turno_enc e , J_prog_turno_det d , JJVDMSCIERREAGOSTO.dbo.J_turnos t   " &
    ...
    Private Sub cargar_operarios(ByVal centro As Integer)
        ...
        If centro = 0 Then
            whereCentro &= " centro IN(1200,2100,2117,2200,2300,2400,2500,2600,2800,3100,3200,3500, 4100, 4110, 4200, 4300, 4500, 5200, 5300, 5400, 6200,6400)"
        Else
            ...
            If (dtg.Columns(j).Name = "hDebeD" Or dtg.Columns(j).Name = "hDebeN" Or dtg.Columns(j).Name = "hDescD" Or dtg.Columns(j).Name = "hDescN" Or dtg.Columns(j).Name = "total" Or dtg.Columns(j).Name = "OrD" Or dtg.Columns(j).Name = "RN" Or dtg.Columns(j).Name = "DD" Or
                    dtg.Columns(j).Name = "DN" Or dtg.Columns(j).Name = "DF" Or dtg.Columns(j).Name = "ED" Or dtg.Columns(j).Name = "EN" _
                         Or dtg.Columns(j).Name = "EDF" Or dtg.Columns(j).Name = "ENF" Or dtg.Columns(j).Name = "DComp" Or dtg.Columns(j).Name = "TotOrD" _
                          Or dtg.Columns(j).Name = "PR" Or dtg.Columns(j).Name = "PRH" Or dtg.Columns(j).Name = "PD" Or dtg.Columns(j).Name = "PE" Or dtg.Columns(j).Name = "PC" _
                          Or dtg.Columns(j).Name = "PL" Or dtg.Columns(j).Name = "PEM" Or dtg.Columns(j).Name = "RDOMF" Or dtg.Columns(j).Name = "PLICR" Or dtg.Columns(j).Name = "D FMLIA" _
                           Or dtg.Columns(j).Name = "PN" Or dtg.Columns(j).Name = "HL" Or dtg.Columns(j).Name = "IR" Or dtg.Columns(j).Name = "IA" Or dtg.Columns(j).Name = "LM" Or dtg.Columns(j).Name = "LICNR" Or dtg.Columns(j).Name = "FN" Or
                           dtg.Columns(j).Name = "V" Or dtg.Columns(j).Name = "DS" Or dtg.Columns(j).Name = "SU") Then
    ...
    Private Sub guarar_liq()
        If Not objRelojLn.verificar_periodo_liquidado(id_periodo) Then
            ...
            Dim fechas_novedad As String = objOpSimplesLn.consultarVal("SELECT fecha_inicial FROM y_periodos_control WHERE idPeriodo = " & id_periodo)
            ...
            sql_gral = "INSERT INTO y_novedades " &
                                        "(nomina,contrato,nit,idperiodo,concepto,fecha,mes,ano,periodo,valor,horas,dias,centro,planta,turno,nro_presta,sumar,oficio,Fecha_Hora_Inicio_Novedad, Fecha_Hora_Final_Novedad) " &
                                        "VALUES "
            If (dtg_consulta.Rows.Count - 1 <> -1) Then
                progBar.Maximum = dtg_consulta.Rows.Count - 1
                progBar.Minimum = 0
                                    ...
                                    If IsNumeric(dtg_consulta.Item(j, i).Value) Then
                                        If (dtg_consulta.Item(j, i).Value > 0) Then
                                            ...
                                            If objRelojLn.validar_fk_novedad(nit, concepto, id_periodo) Then
                                                sql = sql_gral & "(" & nomina & "," & contrato & "," & nit & "," & id_periodo & "," & concepto & ",'" & fecha & "'," & mes & "," & ano & "," & periodo & "," & valor & "," & horas & "," & dias & "," & centro & "," & planta & "," & turno & "," & nro_presta & "," & sumar & "," & oficio & "," & fecha & "," & fecha & " )"
                                            Else
    ...
    Private Sub verificarHorasExtrasProgramadas(ByRef dtg As DataGridView)
        ...
        For i = 0 To dtg.Rows.Count - 2
            ...
            ''PINTAR LOS DD Y LOS DN
            If IsNumeric(dtg.Item("DD", i).Value) Then
                If IsNumeric(dtg.Item("DD", i).Value) Then

                    If (dtg.Item("DD", i).Value > 0) Then
                        dtg.Item("DD", i).Style.BackColor = Color.Cyan
            ...
            If IsNumeric(dtg.Item("DN", i).Value) Then
                If IsNumeric(dtg.Item("DN", i).Value) Then
                    'horas_prog = dtg.Item("DD", i).Value
                    If (dtg.Item("DN", i).Value > 0) Then
                        dtg.Item("DN", i).Style.BackColor = Color.Cyan
    ...
    Private Sub btn_nov_aut_Click(sender As Object, e As EventArgs) Handles btn_nov_aut.Click
        cargar_novedades_aut()
    End Sub
    Private Sub cargar_novedades_no_aut()
        ...
        col_aut_novedad.Visible = True
        col_no_aut_novedad.Visible = True
    ...
    Private Sub cargar_novedades_aut()

        Dim sql As String = "SELECT  n.IdNovedad, t.nit, n.centro , t.nombres, n.concepto, n.idperiodo, n.mes, n.ano, n.horas, n.valor " &
                                "FROM   y_novedades AS n INNER JOIN " &
                                   "V_nom_personal_Activo_con_maquila AS t ON n.nit = t.nit " &
                                        "WHERE  n.concepto IN (13, 14, 15, 16, 31, 32, 33, 35,36, 37, 38, 43, 77, 78, 84, 90, 91, 95, 96, 510, 512, 516, 557) AND n.idperiodo = " & id_periodo
        If (usuario.permiso.Trim <> "ADMIN" And usuario.permiso.Trim <> "NOMINA" And usuario.permiso.Trim <> "DIR_PRODUCCION") Then
    ...
    Private Sub ingresar_novedad()
        If validar_novedad() Then
            ...
            Dim fecha_fin As String = objOpSimplesLn.cambiarFormatoFecha(cbo_fec_fin_novedad.Value)
            ...
            sql = objRelojLn.sql_novedad_no_aut(nit, concepto, id_periodo, fecha, contrato, valor, horas, centro, planta, turno, oficio, observaciones, nom_usuario, fecha_fin)
            listSql.Add(sql)
            If concepto = 90 Then
    ...
    Private Sub dtg_ing_novedad_CellClick(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles dtg_ing_novedad.CellClick
        ...
        If (txt_concepto.Text = "8" Or txt_concepto.Text = "33" Or txt_concepto.Text = "34" Or txt_concepto.Text = "88" Or txt_concepto.Text = "511" Or txt_concepto.Text = "512" Or txt_concepto.Text = "513" Or txt_concepto.Text = "516") Then
            'txt_horas_novedad.Text = "0"
            Label9.Text = "DIAS"
            txt_horas_novedad.Enabled = True
        Else
            Label9.Text = "HORAS"
            'txtValNov.Text = "0"
            txtValNov.Enabled = False
    ...
    Private Sub consolidar_valorizacion()
        ...
        For i = 0 To dtg_valores.Rows.Count - 2
            ...
            For j = 0 To dtg_valores.Columns.Count - 1
                If (dtg_valores.Columns(j).Name = "OrD" Or dtg_valores.Columns(j).Name = "RN" Or dtg_valores.Columns(j).Name = "DD" Or dtg_valores.Columns(j).Name = "DN" _
                            ...
                            Or dtg_valores.Columns(j).Name = "PR" Or dtg_valores.Columns(j).Name = "PRH" Or dtg_valores.Columns(j).Name = "PD" Or dtg_valores.Columns(j).Name = "PC" Or dtg_valores.Columns(j).Name = "PN" _
                            Or dtg_valores.Columns(j).Name = "IR" Or dtg_valores.Columns(j).Name = "IA" Or dtg_valores.Columns(j).Name = "LM" _
                            Or dtg_valores.Columns(j).Name = "DS" Or dtg_valores.Columns(j).Name = "SU" Or dtg_valores.Columns(j).Name = "V" _
        ...
        For j = 0 To dt_consolidado.Columns.Count - 1
            If (dt_consolidado.Columns(j).ColumnName = "OrD" Or dt_consolidado.Columns(j).ColumnName = "RN" Or dt_consolidado.Columns(j).ColumnName = "DD" Or dt_consolidado.Columns(j).ColumnName = "DN" _
                           ...
                           Or dt_consolidado.Columns(j).ColumnName = "PR" Or dt_consolidado.Columns(j).ColumnName = "PRH" Or dt_consolidado.Columns(j).ColumnName = "PD" Or dt_consolidado.Columns(j).ColumnName = "PC" Or dt_consolidado.Columns(j).ColumnName = "PN" _
                           ...
                           dt_consolidado.Columns(j).ColumnName = "V" Or
    ...
    Private Sub DetalleToolStripMenuItem_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DetalleToolStripMenuItem.Click
        ...
        If (dtg_consulta.Columns(colSelect_consulta).Name = "PR" Or dtg_consulta.Columns(colSelect_consulta).Name = "PRH" Or dtg_consulta.Columns(colSelect_consulta).Name = "PD" Or dtg_consulta.Columns(colSelect_consulta).Name = "PE" Or dtg_consulta.Columns(colSelect_consulta).Name = "PC" _
                           Or dtg_consulta.Columns(colSelect_consulta).Name = "PN") Then
            detalle_permisos(nit)
        ElseIf dtg_consulta.Columns(colSelect_consulta).Name = "IR" Or dtg_consulta.Columns(colSelect_consulta).Name = "IA" Or dtg_consulta.Columns(colSelect_consulta).Name = "LM" Then
            detalle_incapacidades(nit)
