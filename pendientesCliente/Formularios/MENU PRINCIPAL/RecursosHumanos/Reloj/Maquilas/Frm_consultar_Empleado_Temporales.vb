...
Public Class Frm_consultar_Empleado_Temporales
    ...
    Private Sub cargar_temporales()
        ...
        sql = "select nit, nombres, direccion, telefono_1, centro, oficio, planta,fecha_ingreso, basico_hora, basico_mes, estado, sexo, turno, desactivar, fecha_retiro, compañia from J_personal_maquila"
    ...
    Private Sub dtg_temporales_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dtg_temporales.CellContentClick
        ...
        If (fila >= 0) Then
            If (col = "nit") Then
                ...
                End If
                'nuevos
                If IsDBNull(dtg_temporales.Item("fecha_ingreso", fila).Value()) = False Then
                    dtp_fecha.Text = dtg_temporales.Item("fecha_ingreso", fila).Value()
                End If
                If IsDBNull(dtg_temporales.Item("turno", fila).Value()) = False Then
                    cbo_turno.SelectedValue = dtg_temporales.Item("turno", fila).Value()
                End If
                If IsDBNull(dtg_temporales.Item("estado", fila).Value()) = False Then
                    cbo_estado.Text = dtg_temporales.Item("estado", fila).Value()
                End If
                If IsDBNull(dtg_temporales.Item("desactivar", fila).Value()) = False Then
                    Textdesactivar.Text = dtg_temporales.Item("desactivar", fila).Value()
                End If
                If IsDBNull(dtg_temporales.Item("fecha_retiro", fila).Value()) = False Then
                    Dtp_fecha_retiro.Text = dtg_temporales.Item("fecha_retiro", fila).Value()
                End If
                If IsDBNull(dtg_temporales.Item("compañia", fila).Value()) = False Then
                    txtcompañia.Text = dtg_temporales.Item("compañia", fila).Value()
                End If
    ...
    Private Sub txt_guardar_Click(sender As Object, e As EventArgs) Handles txt_guardar.Click
        If validar_temporal() Then
            ...
            Dim fecha_retiro As String = Dtp_fecha_retiro.Value.ToString("yyyy/MM/dd")
            Dim descativar As String = Textdesactivar.Text

            Dim sql As String

            If descativar = "" Then
                ''no se va a desactivar
                sql = "update J_personal_maquila set compañia ='" & txtcompañia.Text & "', centro_planta = " & centro & ", desactivar = NULL, fecha_retiro= NULL,estado='" & estado & "', centro=" & centro & ",oficio=" & oficio & ",planta=" & planta & " ,basico_hora=" & txtbasico.Text & ",basico_mes=" & txt_bmes.Text & ",turno=" & turno & ",contrato=1,direccion='" & direccion & "',telefono_1=" & telefono & ",fecha_ingreso='" & fecha_ingreso & "',nombres='" & txt_nombre.Text & "'  where nit=" & nit & ""
            Else
                sql = "update J_personal_maquila set compañia ='" & txtcompañia.Text & "', centro_planta = " & centro & ", desactivar='" & Textdesactivar.Text & "',fecha_retiro='" & fecha_retiro & "',estado='" & estado & "', centro=" & centro & ",oficio=" & oficio & ",planta=" & planta & " ,basico_hora=" & txtbasico.Text & ",basico_mes=" & txt_bmes.Text & ",turno=" & turno & ",contrato=1,direccion='" & direccion & "',telefono_1=" & telefono & ",fecha_ingreso='" & fecha_ingreso & "',nombres='" & txt_nombre.Text & "'  where nit=" & nit & ""
            ...
            'BunifuSnackbar1.Show(Me, "Temporal modificado correctamente",
                        ...
                        If txtbasico.Text <> "" And IsNumeric(txtbasico.Text) Then
                            If txt_bmes.Text <> "" And IsNumeric(txtbasico.Text) Then
                                If cbo_turno.Text <> "TODO" Then
                                    If txtcompañia.Text <> "" Then
                                        resp = True
                                    Else
                                        MessageBox.Show("Seleccione una compañia ", "No ha ingresado basico", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                                    End If
                                Else
